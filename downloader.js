...
async function getStreamUrl(teraboxLink) {
  ...
  // capture mp4 stream from network
  page.on("requestfinished", (req) => {
    const url = req.url();
    if (!streamUrl && url.includes(".mp4")) {
      streamUrl = url;
    }
  });

  try {
    await page.goto("https://teraboxdl.site/", { waitUntil: "domcontentloaded" });
    ...
    // wait a bit for stream to appear
